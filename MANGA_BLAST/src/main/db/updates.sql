...
CREATE TABLE IF NOT EXISTS `categorie` (
  ...
  `colore` varchar(7) DEFAULT '#FF6B35', -- Colore esadecimale per UI
...
-- Le colonne id_categoria e id_sottocategoria esistono già
-- Aggiungere foreign keys se non esistono
...
-- Inserire sottocategorie per Manga
INSERT INTO `sottocategorie` (`id_categoria`, `nome`, `descrizione`) VALUES
-- Shonen
...
-- Inserire sottocategorie per Funko
INSERT INTO `sottocategorie` (`id_categoria`, `nome`, `descrizione`) VALUES
-- Anime
...
-- 4. AGGIORNAMENTI PRODOTTI ESISTENTI
-- =============================================

-- Assegnare categorie ai manga esistenti (esempio)
UPDATE `manga` SET `id_categoria` = 1 WHERE `nome` LIKE '%Naruto%' OR `nome` LIKE '%One Piece%';
...
UPDATE `funko` SET `id_categoria` = 11 WHERE `nome` LIKE '%Anime%' OR `nome` LIKE '%Naruto%';
...
-- Indici per ricerche veloci (creare solo se non esistono)
-- CREATE INDEX IF NOT EXISTS `idx_manga_categoria` ON `manga` (`id_categoria`, `id_sottocategoria`);
-- CREATE INDEX IF NOT EXISTS `idx_funko_categoria` ON `funko` (`id_categoria`, `id_sottocategoria`);
-- CREATE INDEX IF NOT EXISTS `idx_recensioni_prodotto` ON `recensioni` (`id_prodotto`, `tipo_prodotto`, `attiva`);
-- CREATE INDEX IF NOT EXISTS `idx_recensioni_rating` ON `recensioni` (`rating`, `attiva`);

-- 6. VISTE PER FACILITARE LE QUERY
-- =============================================

-- Vista prodotti con categorie
CREATE VIEW `prodotti_con_categorie` AS
SELECT 
    'manga' as tipo,
    m.id,
    m.nome as titolo,
...
CREATE VIEW `statistiche_recensioni` AS
SELECT 
    id_prodotto,
    tipo_prodotto,
